...
// Get global XP pool stats
app.get('/api/stats/global', async (req, res) => {
  try {
    const stats = await enhancedXPService.getGlobalStats();
    res.json(stats);
  } catch (error) {
...
// Get user stats with XP details
app.get('/api/users/:address/stats', async (req, res) => {
  try {
    const { address } = req.params;
    const stats = await enhancedXPService.getUserStats(address);
...
app.post('/api/xp/create-nft', async (req, res) => {
  try {
    const { address, tokenId } = req.body;
    
    if (!address || !tokenId) {
      return res.status(400).json({ error: 'Address and tokenId required' });
    }
    
    const result = await enhancedXPService.awardCreateNFTXP(address, tokenId);
    res.json(result);
  } catch (error) {
    console.error('Error awarding NFT creation XP:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Award XP for swap
app.post('/api/xp/swap', async (req, res) => {
  try {
    ...
    const result = await enhancedXPService.awardSwapXP(address, txHash);
    res.json(result);
  } catch (error) {
    console.error('Error awarding swap XP:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Award XP for daily login
app.post('/api/xp/daily-login', async (req, res) => {
  try {
    ...
    const result = await enhancedXPService.awardDailyLoginXP(address);
...
app.post('/api/xp/share', async (req, res) => {
  try {
    ...
    const result = await enhancedXPService.awardShareXP(address, platform);
...
app.post('/api/xp/genesis-sbt', async (req, res) => {
  try {
    const { address, tokenId } = req.body;
    ...
    const result = await enhancedXPService.awardGenesisSBTXP(address, tokenId);
    res.json(result);
  } catch (error) {
    console.error('Error awarding Genesis SBT XP:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Get token conversion calculation
app.get('/api/token-conversion/:address', async (req, res) => {
  try {
    const { address } = req.params;
    const conversion = await enhancedXPService.calculateTokenConversion(address);
    res.json(conversion);
  } catch (error) {
    console.error('Error calculating token conversion:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
